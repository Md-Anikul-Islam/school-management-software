        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('ebooks.update', $ebook->id) }}" method="POST"
                    ...
                    @method('PUT')
                    <div class="mb-3">
                        <label for="name" class="form-label">Ebook Name</label>
                        <input type="text" class="form-control" id="name" name="name" value="{{ $ebook->name }}"
                               required>
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select select2" id="class_id" name="class_id" required>
                            ...
                            @foreach($classes as $class)
                                <option
                                    value="{{ $class->id }}" {{ $ebook->class_id == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
                            @endforeach
                        </select>
                        @error('class_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="mb-3">
                        ...
                        @if($ebook->cover_photo)
                            <img src="{{ asset('uploads/ebooks/' . $ebook->cover_photo) }}" alt="Cover Photo"
                    ...
                    <div class="mb-3">
                        <label for="file" class="form-label">Ebook File (PDF)</label>
                        <input type="file" class="form-control" id="file" name="file" accept=".pdf">
                        @if($ebook->file)
                            <p class="mt-2">Current File: <a href="{{ asset('uploads/ebooks/' . $ebook->file) }}"
