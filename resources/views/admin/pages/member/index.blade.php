                ...
                <div class="d-flex justify-content-between">
                    <div class="btn-group">
                        <button style="background-color:darkblue;" class="btn text-nowrap text-light"
                                onclick="exportTableToPDF('library_members.pdf', '{{ $pageTitle }}')">
                            Export As PDF
                        </button>
                        <button style="background-color: darkgreen" class="btn btn-info text-nowrap"
                                onclick="exportTableToCSV('library_members.csv')">
                            Export To CSV
                        </button>
                    </div>
                </div>
                ...
                <table id="basic-datatable" class="table table-bordered table-striped dt-responsive w-100">
                    ...
                    @foreach ($students as $student)
                        <tr>
                            ...
                            <td>{{ $student->email }}</td>
                            <td>
                                @if ($libraryMember->where('student_id', $student->id)->count() > 0)
                                    {{ $libraryMember->where('student_id', $student->id)->first()->library->title }}
                                    ({{ $libraryMember->where('student_id', $student->id)->first()->library->code }})
                                @else
                                    No Library
                                @endif
                            </td>
                            <td>
                                @if ($libraryMember->where('student_id', $student->id)->count() > 0)
                                    ...
                                    @endphp
                                    @can('library-member-edit')
                                        <a href="{{ route('library-members.edit', $libraryMember->id) }}"
                                           class="btn btn-info"><i class="ri-edit-line"></i></a>
                                    @endcan
                                    @can('library-member-show')
                                        <a href="{{ route('library-members.show', $libraryMember->id) }}"
                                           class="btn btn-primary"><i class="ri-eye-fill"></i></a>
                                    @endcan
                                    @can('library-member-delete')
                                        <a class="btn btn-danger" data-bs-toggle="modal"
                                           data-bs-target="#danger-header-modal{{ $libraryMember->id }}"><i
                                                class="ri-delete-bin-6-fill"></i></a>
                                        <div
                                            id="danger-header-modal{{ $libraryMember->id }}"
                                            class="modal fade" tabindex="-1"
                                            role="dialog"
                                            aria-labelledby="danger-header-modalLabel{{ $libraryMember->id }}"
                                            aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    <div class="modal-header modal-colored-header bg-danger">
                                                        <h4 class="modal-title"
                                                            id="danger-header-modalLabel{{ $libraryMember->id }}">
                                                            Delete</h4>
                                                        <button type="button" class="btn btn-close btn-close-white"
                                                                data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <h5 class="mt-0">Are you sure you want to delete this library
                                                            member?</h5>
                                                    </div>
                                                    <div class="modal-footer">
                                                        ...
                                                        <form
                                                            action="{{ route('library-members.destroy', $libraryMember->id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('DELETE')
                                    ...
                                    @endcan
                                @else
                                    @can('library-member-create')
                                        <a href="{{ route('library-members.create', $student->id) }}"
